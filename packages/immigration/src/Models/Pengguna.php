...
class Pengguna extends BaseModel
{
	...
	protected $rules				=	[
											...
											// 'visas.*.notaris.id'	=> 'required|max:255',
											// 'visas.*.notaris.nama'	=> 'required|max:255',
	...
	public function grantVisa($visa)
	{
		...
		$exists_visa		= Visa_A::where('immigration_pengguna_id', $this->id)->where('immigration_ro_notaris_id', $visa['notaris']['id'])->first();
		if($exists_visa)
		{
		...
		//3. simpan visa
		//3a. simpan notaris
		$notaris		= new Notaris_RO;
		$notaris_ro		= $notaris->findornew($visa['notaris']['id']);
		$notaris_ro->fill([
			'id' 	=> $visa['notaris']['id'],
			'nama' 	=> $visa['notaris']['nama'],
		]);
		...
		$visa_ag->fill([
			'role'							=> $visa['role'],
			'immigration_ro_notaris_id'		=> $notaris_ro->id,
